...
export default function GatheredSkirtPage() {
  ...
  const validation = validateGatheredSkirt(body, fit, style);
  const net = draftGatheredSkirt(body, fit, style);
  const pattern = withSeamAllowance(net, DEFAULT_SEAM_ALLOWANCE);
  const preview = previewGatheredSkirt(body, fit, style);
