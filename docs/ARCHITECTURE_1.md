...
> ⚠️ This section describes the *intended* structure. Confirm the exact paths and names against the actual code — doing that is a good first task in itself.

- `lib/types/` — the shared data model: `Point`, `Line`, `Millimetres`, body measurements, `OutlinePoint`, `Marking`, `PatternPiece`, `Pattern`, `ConstructionStep`, and the per-garment fit/style types.
- `lib/patterns/` — one module per block (e.g. `gatheredSkirt.ts`) holding its draft, plus shared construction such as `straightWaistband.ts`. Each block module also exports its construction steps.
- `lib/previews/` — the stylised preview generator(s).
- `lib/geometry/` — shared, garment-independent geometry, including the seam-allowance transform.
- `app/` — the Next.js UI: the input form, the flat-pattern SVG renderer, the preview, and the method list.
- `AGENTS.md` — the terse architecture rules for AI coding agents. **This document is its human-facing companion**; keep the two consistent. (One architecture, two renderings for two audiences — which is the project's own philosophy applied to its own docs.)
